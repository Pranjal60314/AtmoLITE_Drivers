module buffered_uart_rx #(
    ...
    // Physical Line
    input  wire       uart_rxd,
    input  wire       uart_rx_en,  // Enable for the RX module logic
    ...
    // 1. Instantiate your UART RX module
    uart_rx #(
        .BIT_RATE(BIT_RATE),
        .CLK_HZ(CLK_HZ)
    ) physical_rx (
    ...
    // 2. Instantiate the FIFO (Receiver Buffer)
    // Note: We write when UART says data is valid.
    fifo #(.DEPTH(FIFO_DEPTH), .WIDTH(8)) rx_buffer (
        .clk(clk),
        .rst(!resetn),           // Active High reset for FIFO
        .wr_en(raw_rx_valid),    // Pulse wr_en whenever a byte is received
        .rd_en(rd_en),           // Controlled by the external system
        .din(raw_rx_data),
        .dout(rd_data),
        .full(fifo_full),
        .empty(fifo_empty)
